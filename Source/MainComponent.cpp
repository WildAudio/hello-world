...
public:
		...
		state(Stopped)
	{
		...
		//Allocate memory for and set the paths for all of the initial files
		files[0] = new File(files[0]->getCurrentWorkingDirectory().getChildFile("Kick.wav"));
		files[1] = new File(files[1]->getCurrentWorkingDirectory().getChildFile("Snare.wav"));
		files[2] = new File(files[2]->getCurrentWorkingDirectory().getChildFile("OHL.wav"));
		files[3] = new File(files[3]->getCurrentWorkingDirectory().getChildFile("OHR.wav"));
		...
		File file2("C:\\Users\\Administrator\\Music\\DrumsForjackDAW\\Kick.wav");
		...
		viewPort = new ViewportComponent(*pTransportSource[0], *pTransportSource[1], *pTransportSource[2], *pTransportSource[3], timer, files);
			...
			if (button == viewPort->getTrackContainer()->getFileSelectWindow()->getOpenButton(i)) {
				...
				if (chooser.browseForFileToOpen())
				{
					...
					if (reader != nullptr)
					{
						...
						ss.str(" ");
						//ss << viewPort->getTrackContainer()->getEditWindow()->getTrack(i)->getWaveform(0)->getEndTime();
						displayLabel.setText(ss.str(), sendNotification);
					}
	...
	void timerCallback() {
		...
		ss.str("");
		ss << files[0]->getCurrentWorkingDirectory().getFullPathName();
		displayLabel.setText(ss.str(), sendNotification);
	}
