...
{
public:
	EditWindow(AudioTransportSource &transportSourceToUse, AudioTransportSource &transportSourceToUse2, AudioTransportSource &transportSourceToUse3, AudioTransportSource &transportSourceToUse4, AudioTransportSource &timerToUse, File *filesToUse[4])
		:
		red(false),
		...
		currentState(0)
	{
		...
		for (int i = 0; i <= 3; i++) {
			if (i == 0) {
				tracks[i] = new AudioTrack(pixelsPerSecond, *files[i], transportSource);
			}
			if (i == 1) {
				tracks[i] = new AudioTrack(pixelsPerSecond, *files[i], transportSource2);
			}
			if (i == 2) {
				tracks[i] = new AudioTrack(pixelsPerSecond, *files[i], transportSource3);
			}
			if (i == 3) {
				tracks[i] = new AudioTrack(pixelsPerSecond, *files[i], transportSource4);
			}
