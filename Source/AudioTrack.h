...
public:
		...
		transportSource(transportSourceToUse)     //...and the transport source reference as the transport source passed
	{
		...
		regions[0]->addChangeListener(this);         //Add a listener so that the regioncan inform the track of when it has been moved
	}
	...
	void updateStartAndEndTimes() {
		...
		regions[0]->setXValueEnd(transportSource.getLengthInSeconds() * pixelsPerSecond);
	}
